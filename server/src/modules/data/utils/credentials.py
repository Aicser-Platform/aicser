...
def _get_fernet():
    """Return a Fernet instance for encrypting/decrypting credentials.

    Encryption is required in every environment - there is no plaintext
    fallback. Local/dev environments get a key auto-generated by
    entrypoint.sh; anything else must set ENCRYPTION_KEY explicitly.
    """
    key = os.getenv("ENCRYPTION_KEY")
    if not key or Fernet is None:
        raise RuntimeError(
            "ENCRYPTION_KEY is not set (or the 'cryptography' package is unavailable). "
            "Credential encryption is required in all environments. Generate one with: "
    ...
    try:
        ...
        raise RuntimeError(
            "ENCRYPTION_KEY is set but invalid; credentials cannot be encrypted/decrypted."
        ) from None
