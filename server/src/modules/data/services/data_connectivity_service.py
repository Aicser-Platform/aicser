...
async def _try_write_bronze(
    ...
    """Open an onboarding session for an upload. EE-only.

    Returns None everywhere else — CE, no organization, or any failure — so the
    blob path is untouched and an upload never fails because onboarding is down.

    Does NOT enqueue the profiling job. `session` is the caller's own,
    not-yet-committed transaction (the parent `data_sources` row is typically
    added to it after this returns), and the ARQ worker looks the onboarding
    session up by id through a *different* database connection. Enqueueing
    here would race the caller's own commit. The caller must enqueue
    "profile_staged_asset" for the returned onboarding_session_id itself,
    only after its own commit has succeeded.
    """
    ...
    try:
        onboarding = await create_session(
            session,
        ...
        return {"onboarding_session_id": str(onboarding.id)}
    except Exception as exc:  # noqa: BLE001 - onboarding must never break upload
        logger.warning(
            "onboarding session could not be opened for %s: %s", data_source_id, exc
        )
        return None
...
def _parse_google_sheet_url(sheet_url: str, config_gid: Optional[Any] = None) -> tuple:
    ...
    if not id_match:
        raise ValueError(
            "Invalid Google Sheet URL: could not extract spreadsheet ID (expect /d/ID in path)"
        )
...
class DataConnectivityService:
    ...
    def __init__(self):
        ...
        self.ai_schema_service = (
            AISchemaService() if AISchemaService else NoOpAISchemaService()
        )
        ...
        self._schema_error_cache_ttl = (
            30  # 30 seconds — failed/timed-out fetch, to stop retry storms
        )
    ...
    async def _test_nosql_connection(
        self, db_type: str, request: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            if db_type == "mongodb" and MONGODB_AVAILABLE and MongoDBConnector:
                ...
                if not conn_str or not database:
                    return {
                        "success": False,
                        "error": "MongoDB requires connection_string (or host) and database.",
                    }
            ...
            if db_type == "cassandra" and CASSANDRA_AVAILABLE and CassandraConnector:
                ...
                contact_points = (
                    [x.strip() for x in host_str.split(",") if x.strip()]
                ...
                if not contact_points or not keyspace:
                    return {
                        "success": False,
                        "error": "Cassandra requires host (contact points) and keyspace/database.",
                    }
                ...
                return {
                    ...
                    if ok
                    else "Cassandra connection failed",
                    "connection_info": {},
                }
            if db_type == "dynamodb" and DYNAMODB_AVAILABLE and DynamoDBConnector:
                ...
                secret_key = request.get("secret_access_key") or request.get(
                    "secretKey"
                )
                if not access_key or not secret_key:
                    return {
                        "success": False,
                        "error": "DynamoDB requires access_key_id and secret_access_key (or accessKey/secretKey).",
                    }
    ...
    async def _test_timeseries_connection(
        self, db_type: str, request: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not handler:
                return {
                    "success": False,
                    "error": f"No handler for connector type: {db_type}",
                }
        ...
        try:
            if db_type == "mongodb" and MONGODB_AVAILABLE and MongoDBConnector:
                ...
                if not conn_str or not database:
                    return {
                        "success": False,
                        "error": "MongoDB config missing connection_string and database.",
                        "tables": [],
                        "schemas": [],
                        "total_rows": 0,
                    }
                    ...
                    tables.append(
                        {
                            ...
                            "row_count": coll_info.get("total_count", 0),
                        }
                    )
                return {
                    "success": True,
                    "tables": tables,
                    "schemas": list(raw.keys()) if isinstance(raw, dict) else [],
            ...
            if db_type == "cassandra" and CASSANDRA_AVAILABLE and CassandraConnector:
                ...
                if not contact_points or not keyspace:
                    return {
                        "success": False,
                        "error": "Cassandra config missing host and keyspace.",
                        "tables": [],
                        "schemas": [],
                        "total_rows": 0,
                    }
                ...
                for table_name, table_info in (raw or {}).items():
                    all_cols = table_info.get("all_columns", []) or table_info.get(
                        "columns", []
                    )
                    col_names = [
            ...
            if db_type == "dynamodb" and DYNAMODB_AVAILABLE and DynamoDBConnector:
                ...
                if not access_key or not secret_key:
                    return {
                        "success": False,
                        "error": "DynamoDB config missing credentials.",
                        "tables": [],
                        "schemas": [],
                        "total_rows": 0,
                    }
                    ...
                    tables = [
                        {
                            "name": table_name,
                            "columns": list(
                                (schema_one or {}).get("attributes", {}).keys()
                            ),
                        }
                    ]
                    return {
                        ...
                        "tables": tables,
                        "schemas": [table_name],
                        "total_rows": (schema_one or {}).get("item_count", 0),
                    }
                # List all tables
                ...
                return {
                    "success": True,
                    "tables": tables,
                    "schemas": table_names[:50],
                    "total_rows": 0,
                }
            return {
                "success": False,
                "error": f"NoSQL schema not implemented for {db_type}.",
                "tables": [],
                "schemas": [],
                "total_rows": 0,
            }
    ...
    async def _convert_upload_to_compressed_parquet(
        ...
        def _detect_csv_header_and_delimiter(
            path: str, fallback_delim: str
        ) -> tuple[int, str]:
        ...
        def _read_into_dataframe() -> pd.DataFrame:
            ...
            if ext in ("csv", "tsv"):
                ...
                if header_row is None:
                    header_row, delimiter = _detect_csv_header_and_delimiter(
    ...
    async def test_database_connection(
        ...
        try:
            ...
            uri_raw = connection_request.get("uri") or connection_request.get(
                "connection_string"
            )
                ...
                try:
                    ...
                    if parsed:
                        connection_request = {
                            ...
                            **{
                                k: v
                                for k, v in connection_request.items()
                                if v is not None
                                and k not in ("uri", "connection_string")
                            },
                        }
            ...
            if db_type in TIMESERIES_TYPES:
                return await self._test_timeseries_connection(
                    db_type, connection_request
                )
            ...
            try:
                test_result = await self.database_connector.test_connection(
                    connection_request
                )
                return test_result
    ...
    async def store_database_connection(
        self, connection_request: Dict[str, Any], user_id: str, project_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"💾 Storing database connection of type '{db_type}' for user {user_id}"
            )
            ...
            try:
                if db_type in NOSQL_TYPES:
                    test_result = await self._test_nosql_connection(
                        db_type, connection_request
                    )
                elif db_type in TIMESERIES_TYPES:
                    test_result = await self._test_timeseries_connection(
                        db_type, connection_request
                    )
                else:
                    test_result = await self.database_connector.test_connection(
                ...
                if not test_result.get("success"):
                    error_msg = (
                        test_result.get("error") or "Database connection test failed"
                    )
                    logger.warning(
                        f"❌ Connection validation failed; not storing data source: {error_msg}"
                    )
                    return {"success": False, "error": error_msg}
            except Exception as validation_error:
                logger.error(
                    f"❌ Unexpected error while validating connection before store: {validation_error}"
                )
                return {
                    "success": False,
            ...
            if db_type not in NOSQL_TYPES and db_type not in TIMESERIES_TYPES:
                try:
                    schema_timeout = float(
                        os.getenv("AICSER_SCHEMA_FETCH_TIMEOUT", "25") or "25"
                    )
                    ...
                    if live_schema.get("success"):
                        ...
                        logger.info(
                            "Initial schema fetch skipped for %s: %s",
                            connection_id,
                            live_schema.get("error"),
                        )
            ...
            try:
                ...
                async with async_session() as db:
                    ...
                    try:
                        from src.modules.data.utils.credentials import (
                            encrypt_credentials,
                        )
                        ...
                        if safe_config != connection_request:
                            logger.info(
                                f"✅ Credentials encrypted for {connection_request.get('type')} connection"
                            )
                        else:
                            logger.warning(
                                f"⚠️ Credentials not encrypted (ENCRYPTION_KEY may not be set) for {connection_request.get('type')} connection"
                            )
                    except Exception as encrypt_error:
                        logger.error(
                            f"❌ Failed to encrypt credentials: {encrypt_error}"
                        )
                    ...
                    if project_id:
                        if isinstance(project_id, str):
                            try:
                                ...
                                logger.warning(
                                    f"Invalid project_id format: {project_id}, storing without project"
                                )
                        else:
                    ...
                    if schema_info.get("tables"):
                        try:
                            ...
                            logger.info(
                                "Relationship auto-detect skipped for %s: %s",
                                connection_id,
                                rel_error,
                            )
                    await db.commit()
            ...
            if db_type not in NOSQL_TYPES and db_type not in TIMESERIES_TYPES:
                try:
                    conn_result = await self.database_connector.create_connection(
                        connection_request
                    )
                    if conn_result.get("success"):
                        logger.info(
                            f"✅ Database connection engine created: {connection_id}"
                        )
                except Exception as conn_error:
                    logger.warning(
                        f"⚠️ Connection engine creation failed (queries may not work): {str(conn_error)}"
                    )
    ...
    async def _read_file_data(
        self, file_path: str, file_format: str, limit: int = None
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # For file and database sources, use MultiEngineQueryService for execution
                ...
                try:
                    ...
                    logger.error(
                        f"❌ MultiEngineQueryService execution failed: {str(query_err)}"
                    )
                    return {
                        "success": False,
    ...
    async def get_data_from_source(
        self, source_id: str, limit: int = None
    ) -> Dict[str, Any]:
            ...
            # For database sources, query the database using MultiEngineQueryService
                ...
                try:
                    ...
                    if schema_info.get("table"):
                        ...
                        schema_info.get("tables") and len(schema_info.get("tables")) > 0
                    ):
                    ...
                    sql = (
                        f"SELECT * FROM {schema_name}.{table_name} LIMIT {limit or 100}"
                    )
        ...
        try:
            ...
            async with async_session() as db:
                ...
                query = select(DataSource).where(
                    DataSource.id == source_id, DataSource.is_active == True
                )
                ...
                if source:
                    source_dict = {
                        ...
                        if getattr(source, "tenant_id", None)
                        else None,
                    ...
                    ) and source.connection_config:
                        try:
                            import json

                            config = (
                                json.loads(source.connection_config)
                                if isinstance(source.connection_config, str)
                                else source.connection_config
                            )
                            ...
                            try:
                                ...
                                logger.debug(
                                    "Connection config loaded for data source %s",
                                    source_id,
                                )
                            except Exception as decrypt_error:
                                logger.warning(
                                    f"⚠️ Could not decrypt credentials for {source_id} (may not be encrypted): {decrypt_error}"
                                )
                            ...
                            source_dict[
                                "metadata"
                            ] = config  # Also add as metadata for compatibility
                        except Exception as config_error:
                            logger.error(
                                f"❌ Error parsing connection_config for {source_id}: {config_error}"
                            )
                    ...
                    if source.type == "sample_duckdb":
                        ...
                        has_tables = isinstance(stored, dict) and bool(
                            stored.get("tables")
                        )
                        if not has_tables:
                            try:
                                schema_result = await self.get_sample_duckdb_schema(
                                    source_dict
                                )
                                if schema_result.get("success") and schema_result.get(
                                    "schema"
                                ):
                                    source_dict["schema"] = schema_result["schema"]
                                    if (
                                        schema_result.get("data_source", {}).get(
                                            "row_count"
                                        )
                                        is not None
                                    ):
                                        source_dict["row_count"] = schema_result[
                                            "data_source"
                                        ]["row_count"]
                                    logger.info(
                                        f"✅ Loaded sample_duckdb schema for {source_id}: {len(schema_result.get('schema', {}).get('tables', []))} tables"
                                    )
                            except Exception as schema_err:
                                logger.warning(
                                    f"⚠️ Could not load sample_duckdb schema for {source_id}: {schema_err}"
                                )
                    ...
                    if source.type == "google_sheets":
                        ...
                        if not has_tables:
                            try:
                                schema_result = await self.get_google_sheets_schema(
                                    source_dict
                                )
                                if schema_result.get("success") and schema_result.get(
                                    "schema"
                                    ...
                                    logger.info(
                                        f"✅ Loaded google_sheets schema for {source_id}"
                                    )
                            except Exception as schema_err:
                                logger.warning(
                                    f"⚠️ Could not load google_sheets schema for {source_id}: {schema_err}"
                                )

                    logger.info(
                        f"✅ Found data source {source_id} in database (type: {source.type}, has_config: {bool(source_dict.get('connection_config'))})"
                    )
                    return source_dict
        ...
        try:
            ...
            async with async_session() as db:
                ...
                if existing:
                    ...
                    existing.file_path = data_source.get(
                        "file_path"
                    )  # Now stores object_key
                    existing.original_filename = data_source.get("original_filename")
                    existing.sample_data = data_source.get(
                        "sample_data"
                    )  # Save sample_data as JSON
                    ...
                    if not project_id and is_ee_enabled():
                        raise ValueError(
                            f"project_id is required when creating data source {data_source.get('id')}. Ensure project_id is passed in options."
                        )
                    ...
                    if isinstance(project_id, str):
                        try:
                            ...
                            raise ValueError(
                                f"Invalid project_id format: {project_id}. Must be a valid UUID."
                            )
                    organization_id_uuid = None
                    if organization_id:
                        try:
                    ...
                    if user_id_str:
                        try:
                            ...
                            logger.warning(
                                f"Could not convert user_id '{user_id_str}' to UUID, skipping"
                            )
                    ...
                    # Flush now, in isolation, so the data_sources row is
                    # physically present before anything that FKs to it
                    # (the onboarding session's staging object, the project
                    # access grant) is added below. SQLAlchemy's unit of work
                    # does NOT order cross-mapper inserts by FK dependency in
                    # this codebase (there are no relationship() mappings
                    # between DataSource and DataLakeObject/
                    # DataOnboardingSession/DataSourceAccessGrant) -- verified
                    # against live Postgres: adding the child rows to the
                    # same session as the parent, even after it, and flushing
                    # them together still raises ForeignKeyViolationError.
                    # Only a separate, earlier flush of the parent avoids it.
                    await db.flush()

                    if is_ee_enabled() and organization_id_uuid:
                        source_schema = data_source.get("schema")
                        all_sheets = (
                            source_schema.get("all_sheets")
                            if isinstance(source_schema, dict)
                            else None
                        )
                        bronze = await _try_write_bronze(
                            db,
                            str(data_source.get("source_file_path") or ""),
                            organization_id_uuid,
                        ...
                        if bronze:
                            if isinstance(data_source.get("schema"), dict):
                                data_source["schema"].setdefault("storage", {})
                                if "onboarding_session_id" in bronze:
                                    data_source["schema"]["storage"][
                                        "onboarding"
                                    ] = bronze
                                else:
                                    data_source["storage_uri"] = bronze["storage_uri"]
                                    data_source["object_key"] = bronze["object_key"]
                                    data_source["storage_format"] = "parquet"
                                    data_source["schema"]["storage"]["bronze"] = bronze
                                # new_data_source.schema is the SAME dict object
                                # as data_source["schema"] (set by reference
                                # above); the mutation above already updated it
                                # in place. But new_data_source was already
                                # flushed once, and a plain JSON column mutated
                                # in place -- even via reassignment of the same
                                # object -- is not picked up as a change by
                                # SQLAlchemy's default (non-mutable) JSON
                                # comparator, so the onboarding info would
                                # silently never reach the database without
                                # this explicit flag.
                                from sqlalchemy.orm.attributes import flag_modified

                                flag_modified(new_data_source, "schema")
                    ...
                    logger.info(
                        f"✅ Saved new data source {data_source['id']} to database (project_id={project_id})."
                    )

                await db.commit()

                if bronze and bronze.get("onboarding_session_id"):
                    # Only now -- after the data_sources row this onboarding
                    # session's FK depends on is durably committed -- is it
                    # safe to let the ARQ worker (a separate DB connection)
                    # go looking for it.
                    try:
                        ...
                        await enqueue_job(
                            ...
                            session_id=bronze["onboarding_session_id"],
                        )
                    except Exception as exc:  # noqa: BLE001 - never break upload
                        logger.warning(
                            "profiling job could not be enqueued for %s: %s",
                            data_source.get("id"),
                            exc,
                        )
            ...
            logger.error(
                f"❌ Failed to save data source to database: {str(e)}", exc_info=True
            )
            import traceback
    ...
    async def process_uploaded_file(
        ...
        try:
            ...
            if file_extension == "csv":
                ...
                data, schema = await self._process_excel_file(
                    file_path, options.get("sheet_name")
                )
            else:
                raise ValueError(f"Unsupported file format: {file_extension}")
            ...
            data_source = {
                "id": options.get("source_id")
                or f"file_{int(datetime.now().timestamp())}",
            ...
            if isinstance(storage_meta, dict) and storage_meta:
                ...
                if isinstance(data_source["schema"], dict):
                    storage_info = {
                        "backend": storage_meta.get(
                            "backend", options.get("storage_type", "postgresql")
                        ),
                        "format": storage_meta.get("storage_format", "parquet"),
                        "uploaded_size_bytes": storage_meta.get(
                            "uploaded_size_bytes", file_size
                        ),
                        "stored_size_bytes": storage_meta.get(
            ...
            if upload_with_prompt:
                ...
                data_source["sample_data"] = (
                    data[:max_sample_rows] if len(data) > max_sample_rows else data
                )
                logger.info(
                    f"⏩ Skipping in-memory storage (file uploaded without prompt)"
                )
            ...
            if not preview_only:
                ...
                if not save_success:
                    logger.error(
                        f"❌ Failed to save data source {data_source.get('id')} to database"
                    )
                    raise Exception(
                        f"Failed to save data source to database. Check logs for details."
                    )
    ...
    async def upload_file(
        ...
        try:
            ...
            result = await self.process_uploaded_file(
                tmp_file_path, filename, options_with_storage, object_key
            )
    ...
    async def upload_file_from_path(
        ...
        logger.info(
            "📁 File upload request (from path): %s (%d bytes)", filename, file_size
        )
        ...
        return await self.process_uploaded_file(
            tmp_file_path, filename, options_with_storage, object_key
        )
    ...
    async def _process_csv_file(
        ...
        try:
            ...
            def _detect_header_row(path: str, fallback_delim: str) -> tuple[int, str]:
                ...
                for delim in candidate_delims:
                    for idx, line in enumerate(lines[:30]):
                        ...
                        consistency = 0
                        for nxt in lines[idx + 1 : idx + 6]:
                            if len((nxt.split(delim) if delim else [nxt])) == len(
                                parts
                            ):
                                consistency += 1
            ...
            if header_row is None:
                ...
                logger.info(
                    f"🔍 Auto-detected CSV header row: {header_row}, delimiter: '{delimiter}'"
                )
            else:
                header_row = max(0, int(header_row))
                logger.info(
                    f"🔍 Using user-provided CSV header row: {header_row}, delimiter: '{delimiter}'"
                )
            ...
            try:
                ...
                schema = {
                    ...
                    "row_count": conn.execute("SELECT COUNT(*) FROM data").fetchone()[
                        0
                    ],
                }
                ...
                sample_result = conn.execute(
                    f"SELECT * FROM data LIMIT {FILE_UPLOAD_SAMPLE_ROWS}"
                ).fetchall()
                ...
                logger.info(
                    f"🦆 Processed CSV with DuckDB: {total_rows} rows, {len(columns)} columns"
                )
                ...
                logger.warning(
                    f"⚠️ DuckDB CSV read failed, falling back to Pandas: {duckdb_error}"
                )
                conn.close()
    ...
    async def _process_parquet_file(self, file_path: str) -> tuple:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"🦆 Processed Parquet with DuckDB: {total_rows} rows, {len(columns)} columns"
                )
                ...
                logger.warning(
                    f"⚠️ DuckDB Parquet read failed, falling back to PyArrow: {duckdb_error}"
                )
                conn.close()
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not read Excel sheet names with openpyxl: {e}, using default"
                    )
                ...
                logger.info(
                    f"📊 Found {len(sheet_names)} sheet(s) in Excel file: {sheet_names}"
                )
                ...
                for idx, sheet in enumerate(sheet_names):
                    try:
                        ...
                        conn.execute(
                            f"CREATE TABLE {table_name} AS SELECT * FROM _temp_{table_name}"
                        )
                        ...
                        logger.info(
                            f"✅ Created DuckDB table '{table_name}' for sheet '{sheet}' ({sheet_schema['row_count']} rows)"
                        )

                    except Exception as sheet_error:
                        logger.warning(
                            f"⚠️ Failed to process sheet '{sheet}': {sheet_error}"
                        )
                        continue
                ...
                primary_schema["duckdb_tables"] = {
                    sheet: info["table_name"] for sheet, info in all_schemas.items()
                }
                primary_schema[
                    "duckdb_connection"
                ] = "in_memory"  # Mark that tables are in DuckDB
                ...
                logger.info(
                    f"🦆 Processed Excel with DuckDB: {len(sheet_names)} sheets, total: {total_row_count} rows"
                )
                ...
                logger.warning(
                    f"⚠️ DuckDB Excel processing failed, falling back to Pandas: {duckdb_error}"
                )
                conn.close()
    ...
    async def _process_json_file(self, file_path: str) -> tuple:
        ...
        try:
            ...
            try:
                ...
                conn.execute(
                    f"CREATE TABLE data AS SELECT * FROM read_json_auto('{file_path}')"
                )
                ...
                logger.info(
                    f"🦆 Processed JSON with DuckDB: {total_rows} rows, {len(columns)} columns"
                )
                ...
                logger.warning(
                    f"⚠️ DuckDB JSON read failed, falling back to Pandas: {duckdb_error}"
                )
                conn.close()
                # Fallback to Pandas if DuckDB fails
                with open(file_path, "r", encoding="utf-8") as f:
    ...
    def _parse_database_uri(self, uri: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(at_parts) < 2:
                ...
                username = (
                    unquote(userinfo[:colon_idx])
                    if colon_idx >= 0
                    else unquote(userinfo)
                )
                password = (
                    unquote(userinfo[colon_idx + 1 :]) if colon_idx >= 0 else None
                )
                port_idx = host_port.rfind(":")
                if port_idx >= 0:
                    host, port_str = host_port[:port_idx], host_port[port_idx + 1 :]
    ...
    async def create_database_connection(
        ...
        try:
            ...
            if db_type not in supported_dbs:
                raise ValueError(
                    f"Unsupported database type: {db_type}. Supported: {supported_dbs}"
                )
    ...
    async def _query_file_data_source(
        ...
        if not data or len(data) == 0:
            ...
            if object_key:
                try:
                    ...
                    if not project_id:
                        logger.warning(
                            "⚠️ project_id not found in data_source, cannot load from datasource storage"
                        )
                    else:
                        logger.info(
                        ...
                        storage_format = (
                            (schema_obj.get("storage") or {}).get("format")
                            if isinstance(schema_obj, dict)
                            else None
                        )
                        blob_format = (storage_format or file_format or "csv").lower()
                        with tempfile.NamedTemporaryFile(
                            delete=False, suffix=f".{blob_format}"
                        ) as tmp:
                        ...
                        try:
                            # Process file
                            data = await self._read_file_data(
                                tmp_path,
                                blob_format,
                                limit=query.get("limit", DEFAULT_FILE_QUERY_LIMIT),
                            )
                            logger.info(
                                "✅ Loaded %d rows from datasource storage", len(data)
                            )
                        finally:
                            # Clean up temp file
        ...
        if not data or len(data) == 0:
            data = data_source.get("sample_data", []) or data_source.get(
                "preview_data", []
            )
            if data:
    ...
    async def _query_database_data_source(
        ...
        db_type = (
            data_source.get("db_type") or data_source.get("format") or ""
        ).lower()
        ...
        if db_type == "mongodb" and MONGODB_AVAILABLE and MongoDBConnector:
            try:
                ...
                nl_query = (
                    query.get("query")
                    or query.get("sql")
                    or query.get("nl_query")
                    or ""
                ).strip() or str(query)
                collection = (
                    query.get("collection") or config.get("collection") or ""
                ).strip()
                if not collection and config.get("database"):
                    collection = (
                        list(connector.db.list_collection_names())[:1][0]
                        if connector.db
                        else ""
                    )
                if not collection:
                    ...
                    return {
                        "success": False,
                        "error": "MongoDB query requires collection name.",
                    }
                result = await connector.execute_nl_query(
                    nl_query, collection, limit=query.get("limit", 1000)
                )
                connector.disconnect()
                return {
                    "success": True,
        ...
        if db_type == "cassandra" and CASSANDRA_AVAILABLE and CassandraConnector:
            try:
                ...
                await asyncio.to_thread(connector.connect)
                nl_query = (
                    query.get("query") or query.get("sql") or query.get("cql") or ""
                ).strip() or str(query)
                ...
                if not table:
                    ...
                    return {
                        "success": False,
                        "error": "Cassandra query requires table name.",
                    }
                result = await connector.execute_nl_query(
                    nl_query, table, limit=query.get("limit", 1000)
                )
                connector.disconnect()
                return {
                    "success": True,
        ...
        if db_type == "dynamodb" and DYNAMODB_AVAILABLE and DynamoDBConnector:
            try:
                ...
                if not table_name:
                    return {
                        "success": False,
                        "error": "DynamoDB query requires table name.",
                    }
                nl_query = (
                    query.get("query") or query.get("sql") or ""
                ).strip() or str(query)
                result = await connector.execute_nl_query(
                    nl_query, table_name, limit=query.get("limit", 1000)
                )
                return {
                    "success": True,
                    "data": result.get("data", []),
            ...
            if not data_source.get("cube_integration"):
                ...
                if not sql_query and isinstance(query, dict):
                    ...
                    if not table:
                        ...
                        if isinstance(schema_info, str):
                            schema_info = json.loads(schema_info)
                        table = schema_info.get("table") or (
                            schema_info.get("tables")[0].get("name")
                            if schema_info.get("tables")
                            else None
                        )
    ...
    async def get_data_source(self, data_source_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"🔍 Data source {data_source_id} not in memory, checking database..."
            )
            import json
            ...
            async with async_session() as db:
                ...
                query = select(DataSource).where(
                    DataSource.id == data_source_id, DataSource.is_active == True
                )
                result = await db.execute(query)
                ...
                if db_source:
                    ...
                    if hasattr(db_source, "file_path") and db_source.file_path:
                        data_source_dict[
                            "file_path"
                        ] = db_source.file_path  # This is now object_key

                    # Parse config and schema if available
                    try:
                        if db_source.connection_config:
                            config = (
                                json.loads(db_source.connection_config)
                                if isinstance(db_source.connection_config, str)
                                else db_source.connection_config
                            )
                            try:
                                from src.modules.data.utils.credentials import (
                                    decrypt_credentials,
                    ...
                    try:
                        if db_source.schema:
                            schema = (
                                json.loads(db_source.schema)
                                if isinstance(db_source.schema, str)
                                else db_source.schema
                            )
                            ...
                            try:
                                ...
                                if schema_result.get("success") and schema_result.get(
                                    ...
                                    if (
                                        ...
                                        data_source_dict["row_count"] = schema_result[
                                            "data_source"
                                        ]["row_count"]
                            except Exception as schema_err:
                                logger.warning(
                                    f"⚠️ Could not load sample_duckdb schema: {schema_err}"
                                )
                        ...
                        if not has_tables:
                            try:
                                ...
                                if schema_result.get("success") and schema_result.get(
                                    ...
                                    logger.info(
                                        f"✅ Loaded google_sheets schema for {data_source_id}"
                                    )
                            except Exception as schema_err:
                                logger.warning(
                                    f"⚠️ Could not load google_sheets schema for {data_source_id}: {schema_err}"
                                )
                    ...
                    if db_source.sample_data:
                        try:
                            sample = (
                                json.loads(db_source.sample_data)
                                if isinstance(db_source.sample_data, str)
                                else db_source.sample_data
                            )
                    ...
                    if (
                        hasattr(db_source, "original_filename")
                        and db_source.original_filename
                    ):
                        data_source_dict[
                            "original_filename"
                        ] = db_source.original_filename

                    # Cache in memory for future access
                    self.data_sources[data_source_id] = data_source_dict

                    return {"success": True, "data_source": data_source_dict}
                else:
                    logger.warning(
                        f"⚠️ Data source {data_source_id} not found in database or memory"
                    )
                                ...
                                if ds_type == "file" and ds_file_path:
                                    ...
                                    if is_blob_key:
                                        try:
                                            ...
                                            project_id_str = (
                                                str(db_source.project_id)
                                                if db_source.project_id
                                                else ""
                                            )
                                            await storage_service.delete_file(
                                                file_path, project_id_str
                                            )
                                            logger.info(
                                                f"✅ Deleted file from datasource storage during data source deletion: {file_path}"
                                            )
                                        except Exception as e:
                                            logger.warning(
                                                f"⚠️ Failed to delete file from datasource storage for data_source {data_source_id}: {e}"
                                            )
                        except Exception as e:
                            logger.warning(
                                f"⚠️ Failed to fetch data source for file deletion: {e}"
                            )
                    ...
                    if is_blob_key:
                        ...
                        if project_id:
                            try:
                                ...
                                logger.info(
                                    f"✅ Deleted file from datasource storage (in-memory cleanup): {file_path}"
                                )
                            except Exception as e:
                                logger.warning(
                                    f"⚠️ Failed to delete file from datasource storage (in-memory): {e}"
                                )
                        else:
                            logger.warning(
                                f"⚠️ Cannot delete file from datasource storage: project_id missing in data_source {data_source_id}"
                            )
    ...
    async def _fetch_api_schema(
        ...
        try:
            ...
            if not config:
                return {
                    "success": False,
                    "error": "No connection configuration for this API source. Edit the data source and set Base URL (and optional Path).",
                }
            try:
            ...
            base = (
                (config.get("url") or config.get("base_url") or "").strip().rstrip("/")
            )
            ...
            if not api_url:
                return {
                    "success": False,
                    "error": "API URL is required. Set Base URL in the data source connection.",
                }
            ...
            if (
                auth_type == "basic"
                and config.get("username") is not None
                ...
                if config.get("api_key"):
                    headers[config.get("api_key_header", "X-API-Key")] = config[
                        "api_key"
                    ]
            ...
            if not rows_for_schema or not isinstance(rows_for_schema[0], dict):
                return {
                    "success": False,
                    "error": "Could not infer schema from API response. Ensure the endpoint returns JSON array or object with 'data'/'results'/'items', or CSV.",
                }
            ...
            logger.info(
                "API schema inferred: %d columns from %s", len(columns), data_source_id
            )
            return {
                "success": True,
                "schema": schema,
    ...
    async def get_sample_duckdb_schema(self, data_source: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            _id = (
                data_source.get("id")
                if isinstance(data_source, dict)
                else getattr(data_source, "id", None)
            )
            _name = (
                data_source.get("name", "")
                if isinstance(data_source, dict)
                else getattr(data_source, "name", "")
            )
            ...
            if domain in _SAMPLE_DUCKDB_SCHEMA_CACHE:
                ...
                if (
                    now - entry.get("ts", 0)
                ) < _SAMPLE_DUCKDB_SCHEMA_CACHE_TTL_SEC and entry.get("schema"):
                    return {
                        "success": True,
                        "schema": entry["schema"],
                        "data_source": {
            ...
            from src.shared.sample_data.generate_duckdb import (
                resolve_sample_duckdb_path,
            )

            sample_path = resolve_sample_duckdb_path()
            if not sample_path:
                logger.debug(
                    "Sample DuckDB file not found in any known location (returning empty schema)"
                )
                return {
                    "success": False,
            ...
            try:
                ...
                logger.info(
                    "Sample DuckDB schema for domain %s: %s tables", domain, len(tables)
                )
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    export_url, timeout=aiohttp.ClientTimeout(total=15)
                ) as resp:
                    if resp.status != 200:
                        text = await resp.text()
                        raise ValueError(
                            f"Google Sheets export returned {resp.status}: {text[:200]}"
                        )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if resp.status != 200:
                        return {
                            "success": False,
                            "error": f"Export returned {resp.status}",
                            "data": [],
                            "row_count": 0,
                        }
                    body = await resp.text()
    ...
    async def get_database_schema(
        self, data_source_id: str, force_refresh: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            async with async_session() as db:
                ...
                if data_source.type == "api":
                    api_result = await self._fetch_api_schema(
                ...
                try:
                    ...
                    config = decrypt_credentials(dict(config))
                    logger.debug(
                        "Decrypted credentials for schema fetch: %s", data_source_id
                    )
                except Exception as decrypt_error:
                    logger.warning(
                        "Could not decrypt credentials for schema fetch (may not be encrypted): %s",
                        decrypt_error,
                    )
                ...
                db_type = (
                    config.get("type") or getattr(data_source, "db_type", "") or ""
                ).lower()
                if db_type == "sqlserver":
                    if not (
                        config.get("username")
                        or str(config.get("username", "")).strip()
                    ):
                        return {
                            "success": False,
                    ...
                    if not (
                        ...
                        and str(config.get("password", "")).strip()
                    ):
                        return {
                            "success": False,
                ...
                _cached_schema = (
                    None if force_refresh else _shared_cache.get(_schema_cache_key)
                )
                if _cached_schema:
                ...
                if (
                    ...
                    and schema_info.get("tables")
                ):
                    logger.info(
                        "✅ Returning stored schema for %s (%d tables)",
                        data_source_id,
                        len(schema_info["tables"]),
                    )
                    _shared_cache.set(
                        _schema_cache_key, schema_info, ttl=self._schema_cache_ttl
                    )
                    return {
                        "success": True,
                        "schema": schema_info,
                ...
                if not force_refresh:
                    ...
                    if _cached_error:
                        logger.info(
                            "⏳ Returning recently-cached schema error for %s (retry backoff)",
                            data_source_id,
                        )
                        return {
                            "success": False,
                ...
                schema_lock = self._schema_fetch_locks.setdefault(
                    data_source_id, asyncio.Lock()
                )
                if schema_lock.locked() and not force_refresh:
                    logger.info(
                        "⏳ Waiting for in-flight schema fetch for %s", data_source_id
                    )
                ...
                async with schema_lock:
                    if not force_refresh:
                        ...
                        if _cached_schema:
                            logger.info(
                                "✅ Returning schema cached by concurrent fetch for %s",
                                data_source_id,
                            )
                            return {
                                "success": True,
                        ...
                        if _cached_error:
                            logger.info(
                                "⏳ Returning schema error cached by concurrent fetch for %s",
                                data_source_id,
                            )
                            return {
                                "success": False,
                    ...
                    try:
                        logger.info(
                            f"🔍 Fetching live schema for {data_source_id}, db_type: {data_source.db_type}, type: {data_source.type}"
                        )
                        schema_timeout = float(
                            os.getenv("AICSER_SCHEMA_FETCH_TIMEOUT", "25") or "25"
                        )
                        ...
                        logger.info(
                            f"📊 Live schema result: success={live_schema.get('success')}, tables_count={len(live_schema.get('tables', []))}, schemas_count={len(live_schema.get('schemas', []))}"
                        )
                        ...
                        if live_schema["success"]:
                            ...
                            logger.info(
                                f"✅ Schema fetched successfully: {len(tables)} tables, {len(schemas)} schemas"
                            )
                            ...
                            await db.flush()
                            try:
                                from src.modules.data.model_service import (
                                    DataModelService,
                                )

                                relationships = await DataModelService(db).auto_detect(
                                    str(data_source.id)
                                )
                                logger.info(
                                ...
                                logger.warning(
                                    "Relationship auto-detect after schema refresh failed: %s",
                                    rel_err,
                                )
                            await db.commit()
                            # Build schema index for Schema RAG (NL2SQL table ranking) on schema refresh.
                            try:
                                from src.modules.ai.services.schema_index_service import (
                                    build_schema_index_for_data_source,
                                )

                                build_result = await build_schema_index_for_data_source(
                                    db, str(data_source.id), updated_schema
                                )
                                await db.commit()
                                logger.info(
                                ...
                                logger.warning(
                                    "Schema index build after schema refresh failed: %s",
                                    idx_err,
                                )
                            ...
                            _shared_cache.set(
                                _schema_error_cache_key,
                                _err,
                                ttl=self._schema_error_cache_ttl,
                            )
                            return {
                                "success": False,
                                "error": _err,
                        ...
                        logger.warning(
                            "Live schema fetch timed out for %s", data_source_id
                        )
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    or "AUTHENTICATION_FAILED" in err_str
                    or "403" in err_str
    ...
    async def _get_enhanced_fallback_schema(
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Fallback schema called for {db_type} - returning empty schema. Real schema should be fetched via database connection."
            )
        ...
        try:
            ...
            if file_format in ["csv", "tsv"]:
                ...
                df_preview = pd.read_excel(
                    file_path, sheet_name=sheets[0], nrows=PREVIEW_ROWS
                )
                preview_data["sample_data"] = df_preview.to_dict("records")
                preview_data["schema_preview"] = {
                    "columns": list(df_preview.columns),
                ...
                if isinstance(json_data, list):
                    ...
                    sample_data = (
                        json_data[:PREVIEW_ROWS]
                        if len(json_data) > PREVIEW_ROWS
                        else json_data
                    )
                else:
                    preview_data["auto_detected_config"] = {
    ...
    def _initialize_demo_data(self):
        ...
        try:
            ...
            self.data_sources["snowflake_warehouse"] = self.data_sources[
                "demo_customers_data"
            ]
            self.data_sources["postgresql_prod"] = self.data_sources[
                "demo_customers_data"
            ]
    ...
    def _get_current_user_id(self) -> int:
        ...
        logger.debug(
            "_get_current_user_id is deprecated; pass user_id from request dependencies"
        )
        return 0
    ...
    async def create_project_data_source(
        ...
        if not user_id:
            raise ValueError(
                "user_id is required for data source creation. Provide it in data_source_data or as a parameter."
            )

        try:
            logger.info(
                f"📊 Creating project data source for project {project_id} (user_id={user_id})"
            )
            ...
            try:
                self.data_sources[generated_id] = {
                    ...
                    "data": data_source_data.get("data")
                    or data_source_data.get("sample_data")
                    or [],
                    "created_at": datetime.now().isoformat(),
                    "updated_at": datetime.now().isoformat(),
                    "organization_id": organization_id,
            ...
            async def _do_create():
                async with async_session() as db:
                    ...
                    try:
                        ...
                        pass
                    db_type = connection_config.get("type") or data_source_data.get(
                        "db_type"
                    )
                    ...
                    data_source_values = {
                        ...
                        "sample_data": data_source_data.get("data")
                        or data_source_data.get("sample_data")
                        or [],
                    ...
                    try:
                        ...
                        with eng_check.connect() as conn:
                            row = conn.execute(
                                sa.text("SELECT id FROM data_sources WHERE id = :id"),
                                {"id": generated_id},
                            ).fetchone()
                            if row and row[0]:
                    ...
                    try:
                        if data_source_values.get("sample_data"):
                            upd = sa.text(
                                "UPDATE data_sources SET sample_data = :sd WHERE id = :id"
                            )
                            with get_sync_engine().begin() as sync_conn:
                                sync_conn.execute(
                                    ...
                                    {
                                        "sd": json.dumps(
                                            data_source_values.get("sample_data")
                                        ),
                                        "id": data_source.id,
                                    },
                                )
                        ...
                        logger.debug(
                            "Failed to persist sample_data for %s", data_source.id
                        )
            ...
            try:
                ...
                def _do_create_sync():
                    try:
                        ...
                        with eng.begin() as conn:
                            ...
                            if project_id_val and isinstance(project_id_val, str):
                                try:
                                    ...
                                    logger.warning(
                                        f"Invalid project_id format: {project_id_val}"
                                    )
                                    project_id_val = None
                            ...
                            params = {
                                ...
                                "schema": json.dumps(ds_vals.get("schema"))
                                if ds_vals.get("schema") is not None
                                else None,
                                "description": ds_vals.get("description")
                                if ds_vals.get("description") is not None
                                else None,
                                "connection_config": json.dumps(
                                    ds_vals.get("connection_config")
                                )
                                if ds_vals.get("connection_config") is not None
                                else None,
                                "file_path": ds_vals.get("file_path")
                                if ds_vals.get("file_path") is not None
                                else None,
                                "original_filename": ds_vals.get("original_filename")
                                if ds_vals.get("original_filename") is not None
                                else None,
                            ...
                            while True:
                                try:
                                    ...
                                    sel = sa.text(
                                        "SELECT id FROM data_sources WHERE id = :id LIMIT 1"
                                    )
                            ...
                            if ds_vals.get("sample_data"):
                                try:
                                    ...
                                    logger.debug(
                                        "Failed to persist sample_data in sync path for %s",
                                        ds_vals.get("id"),
                                    )
                            insert_link = sa.text(
                        ...
                        self.data_sources[ds_vals.get("id")] = {
                            ...
                            "created_at": ds_vals.get("created_at").isoformat()
                            if ds_vals.get("created_at")
                            else None,
                            "updated_at": ds_vals.get("updated_at").isoformat()
                            if ds_vals.get("updated_at")
                            else None,
                            "is_active": True,
                            "organization_id": organization_id,
                            "project_id": project_id,
                ...
                for attempt in range(1, max_enqueue_attempts + 1):
                    try:
                        ...
                        logger.warning(
                            "write_queue.enqueue attempt %d/%d failed: %s",
                            attempt,
                        ...
                        if attempt == max_enqueue_attempts:
                            logger.exception(
                                "write_queue.enqueue failed after %d attempts, falling back to direct async create",
                                max_enqueue_attempts,
                            )
            ...
            return {"success": False, "error": repr(e) or str(e) or "Unknown error"}
    ...
    async def execute_project_data_source_query(
        ...
        try:
            ...
            return await self.execute_query_on_source(
