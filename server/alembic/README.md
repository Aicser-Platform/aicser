...
**For all devs:** Run `alembic upgrade head` (or use the startup script that runs it) so the DB has the latest schema (e.g. `users` onboarding columns: `first_name`, `last_name`, `role`, `onboarding_data`, etc.). Without it you may see "column first_name does not exist" or similar; the app falls back to minimal queries when columns are missing.

When you run the stack with **docker-compose.dev.yml**, migrations run automatically on container start:

1. `chat2chart-server` waits for PostgreSQL, then runs `alembic upgrade head` from `/app`.
2. After migrations succeed, the server starts (uvicorn).

See `docker-compose.dev.yml` (chat2chart-server `entrypoint`) for the exact commands.

### Running Alembic inside the container

From the **repo root** (with the stack running):

```bash
# List current head(s) — should be a single head after merge
docker compose -f docker-compose.dev.yml exec chat2chart-server /venv/bin/python -m alembic -c /app/alembic.ini heads

# Apply all pending migrations (same as on startup)
docker compose -f docker-compose.dev.yml exec chat2chart-server /venv/bin/python -m alembic -c /app/alembic.ini upgrade head

# Show current revision
docker compose -f docker-compose.dev.yml exec chat2chart-server /venv/bin/python -m alembic -c /app/alembic.ini current

# History
docker compose -f docker-compose.dev.yml exec chat2chart-server /venv/bin/python -m alembic -c /app/alembic.ini history
```

The server container uses `working_dir: /app` and the repo is mounted at `/app`, so `alembic.ini` and `alembic/versions/` are from your host (including the merge migration `20260210_merge_heads.py` that unifies multiple heads).
