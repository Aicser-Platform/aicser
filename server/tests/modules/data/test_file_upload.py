...
def mock_data_service():
    ...
    mock.upload_file = AsyncMock(
        return_value={"success": True, "data_source": {"id": "test-id"}}
    )
    return mock
...
def app(mock_data_service):
    ...
    async def upload_file(
        ...
        if any((ds.get("name") or "").lower() == name.lower() for ds in existing):
            raise HTTPException(
                status_code=400, detail="A data source with this name already exists"
            )
...
class TestFileUpload:
    ...
    def test_file_upload_without_project_id(self, client, sample_csv_file):
    ...
    def test_file_upload_without_file(self, client):
        ...
        data = {"name": "Test Data Source", "project_id": "1"}
    ...
    def test_file_upload_excel_file(self, client, mock_data_service, sample_xlsx_file):
        ...
        files = {"file": (filename, io.BytesIO(content), content_type)}
        data = {"name": "Excel Test", "project_id": "1"}
    ...
    def test_file_upload_with_options(self, client, mock_data_service, sample_csv_file):
    ...
    def test_file_upload_preview_only(self, client, mock_data_service, sample_csv_file):
        ...
        files = {"file": (filename, io.BytesIO(content), content_type)}
        data = {"name": "Preview Test", "project_id": "1", "preview_only": "true"}
    ...
    def test_file_upload_duplicate_name(
        ...
        mock_data_service.get_data_sources = AsyncMock(
            return_value=[{"id": "existing", "name": "Test Data Source"}]
        )
    ...
    def test_file_upload_service_failure(
        ...
        files = {"file": (filename, io.BytesIO(content), content_type)}
        data = {"name": "Test Failure", "project_id": "1"}
...
class TestFileUploadEdgeCases:
    ...
    def test_file_upload_with_special_characters_in_name(
        ...
        files = {"file": (filename, io.BytesIO(content), content_type)}
        data = {"name": "Test (Special) [Data]", "project_id": "1"}
    ...
    def test_file_upload_with_numeric_project_id(
        ...
        files = {"file": (filename, io.BytesIO(content), content_type)}
        data = {"name": "Numeric Project", "project_id": "123"}
    ...
    def test_file_upload_exception_handling(
        ...
        files = {"file": (filename, io.BytesIO(content), content_type)}
        data = {"name": "Exception Test", "project_id": "1"}
