  ...
  const loadSavedItems = useCallback(async () => {
    ...
    try {
      ...
      while (offset < total && collected.length < SAVED_FETCH_MAX_ITEMS) {
        ...
        // Break if backend returned no more rows to avoid unnecessary requests.
      ...
      message.error(errorMessage(error, 'Failed to load saved items'));
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <div className="max-w-4xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-[var(--ant-color-text)] tracking-tight">{t('saved_items_title')}</h1>
        </div>
      </div>
