...
import { Button, Input, Mentions, Modal, Typography } from 'antd';
import { DownOutlined, LikeOutlined, UpOutlined } from '@ant-design/icons';
...
const FeedCardComments: React.FC<FeedCardCommentsProps> = ({
  ...
  return (
    <>
      {showCommentBox && (
        <div
          className="p-4 border-t border-[var(--ant-color-border-secondary)] bg-[var(--ant-color-bg-container)] flex flex-col gap-3"
      ...
      {hasCommentListToggle && (
        <button
          type="button"
          className="w-full flex items-center justify-center gap-2 py-2.5 px-4 text-sm font-medium text-[var(--ant-color-text-secondary)] hover:text-[var(--ant-color-text)] transition-colors"
