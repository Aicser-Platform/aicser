...
const eeChatPage = path.resolve(__dirname, 'ee/src/ee/app/(dashboard)/chat/page.tsx');
const eeChatPageFallback = path.resolve(__dirname, 'src/ee-chat-page-fallback.tsx');
...
const chatPageEntry = isEnterprise && existsSync(eeChatPage) ? eeChatPage : eeChatPageFallback;
...
const nextConfig = {
  ...
  experimental: {
    ...
    // imports so only the used code is pulled into the graph. Shrinks the dev
    // module count (and therefore compile memory/time) for these heavy packages.
    ...
    instrumentationHook: sentryEnabled,
    turbo: {
      resolveAlias: {
        ...
        '@/ee/chat-page': isEnterprise
          ? './ee/src/ee/app/(dashboard)/chat/page.tsx'
          : './src/ee-chat-page-fallback.tsx',
        // CE: stub out the EE-only PricingModal so it resolves under Turbopack
        // (mirrors the webpack `@/ee/components/PricingModal` → noop alias below).
        ...(!isEnterprise
